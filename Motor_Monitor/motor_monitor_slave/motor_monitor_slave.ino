...
void printScreen(String master, const uint8_t *data);
/* Classes */
...
public:
  ...
  void onReceive(const uint8_t *data, size_t len, bool broadcast) {
    ...
    printScreen(String(macStr), data);
  }
};
...
void setup() {
  ...
  tft.setTextSize(2);
  tft.print("Searching for masters");
...
void printScreen(String master, const uint8_t *data) {
  ...
  int z = doc["z"];
  int mic = doc["mic"];
  int ambientTemp = doc["ambientTemp"];
  int motorTemp = doc["motorTemp"];
  ...
  tft.drawCircle(tft.getCursorX() + 3, tft.getCursorY() - 6, 2, TFT_WHITE);  // Degree symbol
  tft.println("C");
